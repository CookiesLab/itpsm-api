...
class EloquentDepartment implements DepartmentInterface
{
  ...
  public function __construct(Model $Department, DB $DB)
  {
  ...
  /**
   * Get an Departments by countryId
   *
   * @param  int $id
   *
   ...
   * @return boolean
   */
  public function update(array $data, $department = null)
  {
    if (empty($department)) {
      $department = $this->byId($data['id']);
    }
